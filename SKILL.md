...
通过这个技能操作远端 KB/AIS 时，不要假设本机存在 `kb` 可执行文件。始终把 KB 命令字符串交给 [scripts/kb_execute.py](./scripts/kb_execute.py) 发送到 KB 执行服务。
...
```bash
./scripts/kb_execute.py --url "https://aisdevserver.dev.torchv.com/kb/atomix/execute" --token "xxxxxx" "kb cat --code 2031630406710923264"
```
...
> 若无token，友好引导用户如何获取并告知你：
  1. 获取临时token（不推荐）：在登录的ais网页中使用检查，获取一个请求，从请求头中获取key=token的value
  2. 使用长期token（推荐）: 在登录的ais网页中点击左侧的管理中心 -> 开放密钥 -> 创建密钥，选择开放原子化能力即可得到。最好在当前代理启动环境中添加环境变量
